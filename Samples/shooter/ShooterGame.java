...
TimedBlock.addTimedBlock("ProcessPixel");
        ...
        // Initialize the ParallaxingBackgrounds
        ...
        // Initialize the boundingRectangles list
...
TimedBlock.printTimedBlock();	// TODO: Delete these after testing
        ...
        switch (currentScreen)
        {
            case MainMenu:
            {
                ...
                if (elapsedTimeColor >= 500f)
                {
                    ...
                    if (menuIndex == 0)
                    {
                        if (playColor.equals(Color.White()))
                        {
                            playColor = Color.OrangeRed();
                        }
                        else
                    ...
                    {
                        if (quitColor.equals(Color.White()))
                        {
    ...
    private void updatePlayer(GameTime gameTime)
    {
        ...
        if (currentKeyboardState.isKeyDown(Keys.Space))
        {
            if ((gameTime.getTotalGameTime().getTicks() - previousFireTime.getTicks() > fireTime.getTicks()) & (player.isActive))
            {
                ...
                // playerPosition is in the center of our ship, we have to set our projectile
    ...
    private void updateCollision()
    {
        ...
        // Check if the Player is under the Health and Score text. If so, the Health and Score text
        // will be transparent so we can see the player. We will check which line is longer and use
        // that to test if the player is under the text. We have to offset our player position since
