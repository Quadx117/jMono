...
// TODO(Eric): If the actual value is important and not just the byte
...
public class DxtUtil
{
    ...
    {
        try (MemoryStream imageStream = new MemoryStream(imageData))
        {
            return decompressDxt1(imageStream, width, height);
        }
    }

    protected static byte[] decompressDxt1(Stream imageStream, int width, int height)
    {
    ...
    private static void decompressDxt1Block(BinaryReader imageReader, int x, int y, int blockCountX, int width, int height, byte[] imageData)
    {
        ...
        // TODO(Eric): Out parameters of primitive data types doesn't work in Java
    ...
    protected static byte[] decompressDxt3(Stream imageStream, int width, int height)
    {
        byte[] imageData = new byte[width * height * 4];
    ...
    {
        // NOTE(ERIC): Since java doesn't have unsigned values, we need to use a
    ...
    protected static byte[] decompressDxt5(Stream imageStream, int width, int height)
    {
        byte[] imageData = new byte[width * height * 4];
